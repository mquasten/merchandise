...
public class SubjectMapperTest {
	...
	public final void convertNulls() {
		...
		Assert.assertTrue(item.getItemPropertyIds().contains(SubjectCols.Name));
		Assert.assertTrue(item.getItemPropertyIds().contains(SubjectCols.Description));
		item.getItemPropertyIds().forEach(id -> Assert.assertEquals("", item.getItemProperty(id).getValue()));
