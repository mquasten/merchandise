...
public abstract class DocumentModelAO implements Serializable {
	...
	@MethodInvocation(actions={@ActionEvent(params={@Parameter(clazz = DocumentModelAO.class, proxy=true ), @Parameter(clazz = FacesContextFactory.class , el="#arg.facesContext().externalContext.requestParameterMap['documentId']" , elResultType=Long.class , converter=String2LongConverter.class),  @Parameter(clazz = FacesContextFactory.class , el="#arg.facesContext().externalContext.requestParameterMap['page']" , elResultType=String.class ), @Parameter(clazz = FacesContextFactory.class , el="#arg.facesContext().externalContext.requestParameterMap['selected']" , elResultType=String.class ),  @Parameter(clazz = FacesContextFactory.class , el="#arg.facesContext().externalContext.requestParameterMap['returnPage']" , elResultType=String.class )}) , @ActionEvent(name="calculateSize" , params={@Parameter(clazz=DocumentModelAO.class, proxy=true)})}, clazz = DocumentControllerImpl.class)
	@PostConstruct
